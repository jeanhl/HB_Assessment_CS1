def string_compare(s1, s2):
    ...
    # iterating through a list: N
    for i in range(len(s1)):
        # taking an index is constant...
        if s1[i] != s2[i]:
            return False
...
def sum_zero_1(numbers):
    ...
    s = set(numbers) # plus N

    # N
    for x in s:
        # times N (this if statement loops through the list s again)
        if -x in s:
            result.append([-x, x]) # plus N
